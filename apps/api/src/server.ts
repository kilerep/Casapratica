import { z } from "zod";
import { AIChatService, createCasaPraticaAgentSystem, loadAgentPrompts, OpenAIManagerRunner, registerContentTools, registerPinterestStrategyTools, registerProductResearchTools, ToolRegistry } from "@casapratica/agents";
import { createPrismaClient, PrismaContentRepository, PrismaConversationSessionRepository, PrismaIntegrationRepository, PrismaOAuthStateRepository, PrismaPinterestStrategyRepository, PrismaResearchRepository, PrismaTraceRepository } from "@casapratica/database";
import { FetchHttpClient, IntegrationProviderRegistry, IntegrationService, MercadoLivreProductProvider, PinterestBoardProvider, createMercadoLivreProvider, createMetaProvider, createPinterestProvider } from "@casapratica/integrations";
import { TokenCipher, encryptionKeySchema } from "@casapratica/security";
import { ContentEngine, PinterestStrategyEngine, ProductResearchService } from "@casapratica/strategy";
...
const app = buildApp({ ...(aiChat ? { aiChat } : {}), ...(integrations ? { integrations } : {}), ...(content ? { content } : {}), ...(pinterest ? { pinterest } : {}), ...(env.DEFAULT_WORKSPACE_ID ? { workspaceId: env.DEFAULT_WORKSPACE_ID } : {}) });
