import { z } from "zod";
import { AIChatService, createCasaPraticaAgentSystem, loadAgentPrompts, OpenAIManagerRunner, registerContentTools, registerProductResearchTools, ToolRegistry } from "@casapratica/agents";
import { createPrismaClient, PrismaContentRepository, PrismaConversationSessionRepository, PrismaIntegrationRepository, PrismaOAuthStateRepository, PrismaResearchRepository, PrismaTraceRepository } from "@casapratica/database";
import { FetchHttpClient, IntegrationProviderRegistry, IntegrationService, MercadoLivreProductProvider, createMercadoLivreProvider, createMetaProvider, createPinterestProvider } from "@casapratica/integrations";
import { TokenCipher, encryptionKeySchema } from "@casapratica/security";
import { ContentEngine, ProductResearchService } from "@casapratica/strategy";
...
const app = buildApp({ ...(aiChat ? { aiChat } : {}), ...(integrations ? { integrations } : {}), ...(content ? { content } : {}), ...(env.DEFAULT_WORKSPACE_ID ? { workspaceId: env.DEFAULT_WORKSPACE_ID } : {}) });
