...
export class OAuthProvider implements IntegrationProvider {
  ...
  private async exchange(body: URLSearchParams) { const r = await this.http.request<R>({ url: this.c.tokenEndpoint, method: "POST", headers: { "Content-Type": "application/x-www-form-urlencoded" }, body }); return { accessToken: r.access_token, refreshToken: r.refresh_token ?? null, expiresAt: new Date(Date.now() + r.expires_in * 1000), scopes: (r.scope ?? "").split(/[ ,]+/).filter(Boolean), externalAccountId: r.user_id === undefined ? null : String(r.user_id) } satisfies OAuthTokens; }
}
