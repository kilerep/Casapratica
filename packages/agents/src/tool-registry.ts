...
export interface CreativeStudioToolService { request(workspaceId: string, productId: string, input: CreativeInput): Promise<CreativeRequest>; analyze(request: CreativeRequest): unknown; select(request: CreativeRequest): unknown; preview(request: CreativeRequest): Promise<unknown>; render(request: CreativeRequest): Promise<unknown>; variants(request: CreativeRequest): unknown; validate(request: CreativeRequest): unknown; getProductCreatives(workspaceId: string, productId: string): Promise<unknown> }
export function registerCreativeStudioTools(registry: ToolRegistry, service: CreativeStudioToolService, workspaceId: string): void { const parameters = z.object({ productId: z.string().min(1), contentId: z.string().min(1), platform: z.enum(["pinterest", "facebook"]), format: z.enum(["PINTEREST_2_3", "FACEBOOK_4_5", "FACEBOOK_1_1"]), template: z.enum(["MINIMAL_OVERLAY", "BOTTOM_INFO_PANEL", "TOP_HEADLINE", "SIDE_INFO", "PHOTO_FIRST"]).default("PHOTO_FIRST"), variantCount: z.number().int().min(1).max(3).default(1) }); const load = async (input: z.infer<typeof parameters>) => service.request(workspaceId, input.productId, { contentId: input.contentId, platform: input.platform, format: input.format, template: input.template, variantCount: input.variantCount }); registry.register({ name: "analyze_product_images", description: "Analisa imagens reais com score explicável.", parameters, externalAction: false, execute: async input => service.analyze(await load(input)) }); registry.register({ name: "select_best_product_image", description: "Seleciona deterministicamente a melhor imagem real.", parameters, externalAction: false, execute: async input => service.select(await load(input)) }); registry.register({ name: "create_creative_preview", description: "Compõe preview local; não publica.", parameters, externalAction: false, execute: async input => service.preview(await load(input)) }); registry.register({ name: "render_product_creative", description: "Renderiza e persiste CreativeAsset sem publicar.", parameters, externalAction: false, execute: async input => service.render(await load(input)) }); registry.register({ name: "create_creative_variants", description: "Cria variações com objetivos editoriais distintos.", parameters, externalAction: false, execute: async input => service.variants(await load(input)) }); registry.register({ name: "validate_creative", description: "Valida imagem, preço, claims, crop, contraste e overlays.", parameters, externalAction: false, execute: async input => service.validate(await load(input)) }); registry.register({ name: "get_product_creatives", description: "Lista CreativeAssets persistidos do produto.", parameters: z.object({ productId: z.string().min(1) }), externalAction: false, execute: ({ productId }) => service.getProductCreatives(workspaceId, productId) }); }

export interface ContentOperationsToolService { run(date: Date): Promise<unknown>; listQueue(): Promise<unknown>; approve(id: string, actorId: string): Promise<unknown>; reject(id: string, actorId: string, reason: string): Promise<unknown>; approveBatch(ids: readonly string[], actorId: string): Promise<unknown>; schedule(id: string, actorId: string, at: Date): Promise<unknown>; publishNow(id: string, actorId: string): Promise<unknown>; cancel(id: string, actorId: string): Promise<unknown>; retry(id: string, actorId: string): Promise<unknown>; status(id: string): Promise<unknown>; alerts(): Promise<unknown> }
export function registerContentOperationsTools(registry: ToolRegistry, service: ContentOperationsToolService): void { const id = z.object({ id: z.string().min(1), actorId: z.string().min(1) }); registry.register({ name: "run_daily_operations", description: "Prepara o plano e a fila do dia sem publicar.", parameters: z.object({ date: z.coerce.date() }), externalAction: false, execute: ({ date }) => service.run(date) }); registry.register({ name: "list_publication_queue", description: "Lista a fila de aprovação.", parameters: z.object({}), externalAction: false, execute: () => service.listQueue() }); registry.register({ name: "approve_publication_item", description: "Aprovação humana explícita.", parameters: id, externalAction: true, execute: ({ id, actorId }) => service.approve(id, actorId) }); registry.register({ name: "reject_publication_item", description: "Rejeição humana explícita.", parameters: id.extend({ reason: z.string().min(1) }), externalAction: true, execute: ({ id, actorId, reason }) => service.reject(id, actorId, reason) }); registry.register({ name: "approve_publication_batch", description: "Aprovação humana em lote.", parameters: z.object({ ids: z.array(z.string()).min(1), actorId: z.string() }), externalAction: true, execute: ({ ids, actorId }) => service.approveBatch(ids, actorId) }); registry.register({ name: "schedule_publication", description: "Agenda item aprovado.", parameters: id.extend({ scheduledAt: z.coerce.date() }), externalAction: true, execute: ({ id, actorId, scheduledAt }) => service.schedule(id, actorId, scheduledAt) }); registry.register({ name: "publish_now", description: "Publicação externa protegida por aprovação.", parameters: id, externalAction: true, execute: ({ id, actorId }) => service.publishNow(id, actorId) }); registry.register({ name: "cancel_publication", description: "Cancela item publicável.", parameters: id, externalAction: true, execute: ({ id, actorId }) => service.cancel(id, actorId) }); registry.register({ name: "retry_publication", description: "Retry controlado e idempotente.", parameters: id, externalAction: true, execute: ({ id, actorId }) => service.retry(id, actorId) }); registry.register({ name: "get_publication_status", description: "Consulta status real persistido.", parameters: z.object({ id: z.string() }), externalAction: false, execute: ({ id }) => service.status(id) }); registry.register({ name: "list_operational_alerts", description: "Lista alertas operacionais.", parameters: z.object({}), externalAction: false, execute: () => service.alerts() }); }
