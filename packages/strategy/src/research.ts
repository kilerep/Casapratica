...
export interface RankedCandidate { product: MarketplaceProduct; score: ScoreResult; opportunityScore?: OpportunityScore; verdict: Verdict; reasons: readonly string[]; comparisonGroup: string }
...
export class ProductResearchService {
  ...
  async research(request: ResearchRequest) {
    ...
    const candidates = products.map(product => { const comparisonGroup = comparableGroupFor(product), group = groups.get(comparisonGroup) ?? [product], score = scoreCandidate(product, group), verdict = verdictFor(product, score, group), opportunityScore = opportunityByProduct.get(product.externalId); const reasons = [score.explanation, product.price !== null && (product.price < 30 || product.price > 500) ? "Preço fora da faixa preferencial de R$30 a R$500; avaliado pelo contexto, sem rejeição automática." : "Preço dentro da faixa preferencial ou ausente.", group.length >= 3 ? `Comparado com ${group.length} anúncios equivalentes.` : `Somente ${group.length} anúncio(s) equivalente(s) disponível(is).`, product.missingFields.length ? `Dados ausentes: ${product.missingFields.join(", ")}.` : "Campos essenciais disponíveis."]; return { product, score, ...(opportunityScore ? { opportunityScore } : {}), verdict, reasons, comparisonGroup }; }).sort(candidateOrder);
